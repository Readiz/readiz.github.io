...
- BOJ 회의실 배정 문제: https://www.acmicpc.net/problem/1931
...
### 동시에 할 수 없는 경우에는 새로운 기계를 배정하는 경우

- BOJ 강의실 배정 문제: https://www.acmicpc.net/problem/11000
...
void solve() {
    sort(v.begin(), v.end()); // 강의를 시작하는 순서대로 정렬
    ...
    for(auto& item: v) {
        ...
        if (item.l >= pq.top()) {
            // 들어갈 수 있다. (가장 빨리 끝나는 녀석이 top에 있음)
            pq.pop();
            pq.push(item.r); // 기존 녀석을 대체
        } else {
            // 못들어가는 case이다. (현재 가장 빨리 끝나는 녀석보다도 시작이 빠름)
            pq.push(item.r);
        }
    }
