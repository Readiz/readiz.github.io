...
- 대회 참가 유무: Y
- 최종 Performance: <span style={{color:'gray'}}>**1182**</span> (Rank: 3547 / 18717)
...
두 번째 case가 tricky한데, 사실 이는 예전 `지수와 로그` 단원에서 가끔 문제로 나오던 성질이라서 기억이 났다. $b_i = 2^{a_i}$로 정의 했으므로, $a_i = 1, 2$가 되는 경우만 특별하게 세어 주고 (1과 2가 나온 숫자의 수를 전처리 해주면 된다), 숫자가 같은 경우의 수는 간단하게 $_nC_2$로 구하면 된다.
...
그래프 문제. G의 경우 할만해 보였는데 접근 방법이 마지막까지 떠오르지 않아 풀이에 실패했다. 이 두 문제도 해보는게 좋을 듯.
