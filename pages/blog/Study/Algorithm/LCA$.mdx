...
만약 `depth`를 안다면, `l`과 `r`의 `depth`를 같게 맞춘 이후에, 양쪽 `Node`를 만날 떄까지 같이 올리면 된다. 구현의 편의를 위해 처음에는 무조건 `l`의 `depth`가 크도록 `swap`을 하는 것이 편하다. 이렇게 구현하면 2중포문이 없기 때문에 `worst`는 `O(n)`이다.
...
새로운 값이 추가될 때, 위와 같이 `Node`를 업데이트 하면 된다.
