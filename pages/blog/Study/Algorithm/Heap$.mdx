...
이게 좀 어렵다. 우선, 1번 `index`의 데이터가 빠져나간다. 그리고 나서 swap이 시작될 텐데, 일단 이 때의 target은 `sz - 1` 위치에 있는 아이템이 될 것이다. 먼저 좌측 자식으로 swap 한다고 생각한 이후, 우측 자식의 조건이 더 좋으면 우측 자식하고 swap 한다.
