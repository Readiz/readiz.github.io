...
이게 좀 어렵다. 우선, 1번 `index`의 데이터가 빠져나간다. 그리고 나서 swap이 시작될 텐데, 일단 이 떄의 target은 `sz - 1` 위체이 있는 아이템이 될 것이다. 먼저 좌측 자식으로 swap 한다고 생각한 이후, 우측 자식의 조건이 더 좋으면 우측 자식하고 swap 한다.
...
Heap에 삽입된 시점의 `Index`를 저장해주면, 특정 원소의 값을 찾을 수 있고, 그 `Index`의 값을 변경할 수 있다. 변경 후에는 `update`와 `downdate`를 순서 관계없이 1번씩 해주면 다시 heap의 구조가 유지된다. 이를 활용한 전체 heap 코드의 샘플은 아래와 같다.
