...
주의해야할 점은 정렬할 때 인데, 밀도의 계산을 분수로 하게 되면 `WA`를 받기 쉽다. 이 경우 분수로 하기보다는 $a / b$와 $c / d$를 비교한다 할 때 $a * d$와 $b * c$를 비교하는 식으로 회피하면 실수 오차 없이 정렬할 수 있다. 또, 이 경우 문제 조건에서 수의 범위가 `int` 범위를 초과할 수 있으므로, `long long`을 사용해야 한다.
