...
2중 루프를 도는 풀이로 보여서 시간복잡도가 $O(N^2)$으로 보이지만, 사실 `substr`의 시간복잡도가 $O(N)$이기 때문에, 아래 샘플 코드의 전체 시간복잡도는 $O(N^4 \log N)$이 된다. 여기서 로그는 `set`을 사용하기 때문에 붙었다. 하지만 이 비효율적인 시간복잡도에도 문제 조건에서 $N \leq 100$ 이기 때문에 충분히 통과하는 코드이다.
