...
SIMD는 본래 Sinlge Instructgion Multiple Data의 약자이다. `Vectorization`과 관련있는데, 보통 그래픽 세계에서는 하나의 `Instruction`으로 여러개의 위치에 있는 데이터를 동시에 조작할 필요가 있다. 예를 들어, 색상을 반전시키는 operation의 경우 전체 픽셀에 대해 색상 비트를 뒤집어야 할 것이다. 이러한 작업들은 꼭 그래픽에 국한되지 않더라도, 일반적으로 필요한 경우들이 많이 있다. 그래서 예전부터 major한 CPU들에서 지원이 이루어지고 있으며, Intel CPU는 이를 `avx` 라는 이름으로 지원한다.
...
말만 들어서는 병렬연산 체감이 잘 안될 수 있는데, 구체적인 예시를 들어보면 `256 bits` 단위를 사용할 경우를 기준으로 16비트 정수(`short`)를 기본 자료형으로 사용하는 경우, 동시에 `size`가 16인 `vector`에 연산이 가능하다는 뜻이다. 시간복잡도가 정직하게 $O(N/16)$으로 줄어드는 효과를 볼 수 있겠다. 간혹가다가 `bitset`을 비슷하게 사용하는 느낌이 있는데, 이와 유사하다.
