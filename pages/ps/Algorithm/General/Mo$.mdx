...
예를 들어, 전체 구간 크기가 `10000`이면, 각 구간은 `100`으로 나눌 수 있고, 이 상태에서 각 구간의 합을 `bucket[i/100]`에 미리 계산해 두었다고 하자. 그러면, $[4, 303]$ 구간의 합은 미리 계산해둔 값들을 바탕으로 아래처럼 구하면 된다.
...
바로 이러한 루트로 나누는 아이디어를 `Offline query`에 적용한 것이 `Mo's Algorithm`이다. `Offline query` 역시 발상이 독특한데, `query`를 꼭 입력이 들어온 순서대로 처리하지 않는 것을 말한다. 만약 아래와 같은 `query`가 반복된다고 해보자.
...
딱 봐도 `query`를 재배치하고 싶은 욕구가 생길 것이다. 재배치하면 이전에 계산한 값을 활용할 여지가 있기 때문이다. 다만, 재배치를 `일반적으로` 어떤 순서로 할 것인지가 막막하다. `Sqrt Decomposition`까지는 루트를 어떻게 활용할지에 대한 아이디어가 어느 정도 직관으로 찾아지지만, `Mo's` 정도 되면 쉽지 않다. 재배치 방식에 대해 아래처럼 생각을 해보는 것이 일반적일 것이다.
