...
PBDS는 Policy based data structures의 약자로, `g++`에서 `ext/pb_ds` 아래에 위치하는 데이터 구조를 칭한다. (MSVC에서는 사용이 어려울 듯) 보통 `PS`에서 이것을 사용하는 의의 중 하나는 기존 `STL Set`에서 지원하지 않는 `K`번째 크기가 큰(또는 작은) 원소를 빠르게 찾을 수 있게 하는 데이터 구조를 사용하기 위해 사용하는 것이다.
